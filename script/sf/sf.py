...
try:
    ...
    hadsend = True
    print("✅ 已加载notification模块")
except ImportError as e:
    hadsend = False
    print(f"⚠️  未加载notification模块: {e}")
...
class RUN:
    def __init__(self, info, index):
        ...
        url = split_info[0]
        # 自动解码被编码的URL (处理 https%3a%2f%2f 这种情况)
        if '%' in url:
            try:
                url = unquote(url)
            except:
    ...
    def sendMsg(self, help=False):
        ...
        if hadsend and one_msg:
            try:
                ...
                title = f'顺丰速运 - 账号{self.index}'
                send_notification(title, one_msg.strip())
...
详细结果请查看各账号单独通知"""
                send_notification('顺丰速运汇总', summary_msg)
                print('✅ 汇总通知发送完成')
            except Exception as e:
                print(f'❌ 汇总通知发送失败: {e}')
    else:
